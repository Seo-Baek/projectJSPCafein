...
<body>
	...
	<div class="container" align="center" style="margin-right: %15;">
				...
				<tr>
					<td>
						<div class="contianer reply">
							...
							<c:if test="${reply.getWriternum() <= 100}">
							<c:if test="${sessionScope.userNo > 100 }">운영자</c:if>
							<c:if test="${sessionScope.userNo <= 100 }">${reply.getWriter() }  _운영자</c:if>
		...
		<div class="row" style="margin-bottom: 3%;">
				<div class="row" style="margin-bottom: 3%;">
				<c:if test="${sessionScope.userNo <= 100 }">
				<div class="col-sm-7 col-sm-offset-2" align="center">
					<input class="btn contbutton update" type="button" value="글수정"
						style="margin-top: 1.9%; margin-left: 7.5%;" />
					<input type="button" class="btn contbutton showing"value="글노출"
						style="margin-top: 1.9%; margin-left: 7.5%;" />
				</div>
				</c:if>
		</div>
...
<script type="text/javascript">
	...
	$(".showTr").click(function(){
		...
		$(".update").click(function(){
				...
				let category = 'notice';
				location.href="getNoticeCont.do?board_no="+board_no+"&&category="+category;			
		});
		
		$(".showing").click(function(){
			let board_no = $("#board_no").val();
			window.open("notice_show.do?board_no="+board_no,"공지사항 노출 - 재확인",
					"screenY=400, screenX=730,location=no,resizable=no,width=450,height=300");
		});
   ...
   $(".del").click(function(){
	   ...
	   if($("#can"+comNo).text() == "취소"){
		   ...
		   let ok = jConfirm('댓글을 삭제하시겠습니까?','확인', function(result){
			   if(result){
				 let board_no = $("#board_no").val();
			  	 location.href="delRep.do?rep_no="+comNo+"&board_no="+board_no+"&category=공지사항";			   
			   } else {
				   console.log(board_no);
			   }
