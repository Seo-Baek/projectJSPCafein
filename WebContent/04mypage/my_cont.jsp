	...
	<div class="container" align="center" style="margin-right: %15;">
		...
		<div align="right" style="padding-right: 3%;">
			<input class="btn contbutton" type="button" value="목록" 
				onclick="location.href='enter_list.do?mno=${sessinScope.userNo}&accept=1'" />
		</div>
		<br />
		<div>
		...
		<form method="post" action="man_allow.do">
			<input type="hidden" id="title"name="title" value="${dto.getBoard_title() }" />
			<input type="hidden" id="board_no" name="board_no" value="${dto.getBoard_no() }" />
...
<script type="text/javascript">
	...
	$(".update").click(function(){
		...
		location.href="info_update.do?board_no="+board_no+"&&category=mine";			
	});
	
		$(".delete").click(function(){
			let board_no = $("#board_no").val();
			let board_pwd = $("#pwd").val()
			/* location.href="info_delete.do?board_no="+board_no+"&&category='mine'"; */
			window.open("info_delete.do?board_no="+board_no+"&&category=mine","비밀번호 확인 - 삭제",
