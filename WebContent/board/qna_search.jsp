                 ...
                 <c:forEach items="${list }"  var="dto">
                     <tr >
                        ...
                        <td  class="array">
	                        ...
	                        <a href ="qna_searchCont.do?no=${dto.getQna_no() }&page=${page}&field=${find_field}&name=${find_name}">
	                          ${dto.getQna_title() }</a>
	                     </td>
              ...
              <c:if test ="${empty list }">
               ...
               <tr >
                   <td colspan="5" class="array2">
                         <input type="button"   class="btn btn-defult" value="전체목록"  onclick="location.href='<%=request.getContextPath() %>/qna_list.do'">
                   </td>
               </tr>        
    ...
    <!-- 이전 페이지 처리하기 -->
            <c:if test= "${page >block }">
               <li class="paginate_button previous">
                  <a href="qna_search.do?page=1&find_field=${find_field}&find_name=${find_name}">◀◀</a> </li>
               <li> <a href="qna_search.do?page=${startBlock-1 }&find_field=${find_field}&find_name=${find_name}">◀</a></li>  
           </c:if>
           
           <!-- 현재페이지를 블럭에 표시 해 주기 -->
           <c:forEach begin="${startBlock }" end="${endBlock }" var="i">
                 <c:if test="${i==page}" >
                      <li class="active"> <a href="qna_search.do?page=${i }&find_field=${find_field}&find_name=${find_name}">${i }</a> </li>    
                  </c:if>
                  
            <!-- 현재 페이지가 아닌 다른 블록들을 보여주기 위한 기능. -->          
            <c:if test="${i != page }">
                <li> <a href="qna_search.do?page=${i }&find_field=${find_field}&find_name=${find_name}">${i }</a></li>
           </c:if>
           </c:forEach>
           
           <!-- 다음 페이지로 이동 -->
           <c:if test = "${endBlock < allPage }">
              <li><a href="qna_search.do?page=${endBlock + 1 }&find_field=${find_field}&find_name=${find_name}">▶</a></li>
              <li class="paginate_button next">
                 <a href="qna_search.do?page=${allPage}&find_field=${find_field}&find_name=${find_name}">▶▶</a></li>
             </c:if>    
           </ul>
           </div>  
           
           <!-- 검색기능 -->
           <div align="left" style=margin-left:150px;margin-top:30px;>
