		...
		<div class="text-center">
			<ul class="pagination">
				<c:if test="${page > block }">
					...
					<li>
						<a href=
						"info_search.do?page=${startBlock-1 }&&mno=-1&&accept=1&&req_box=${req_box}&&req_text=${req_text}"
							>◀</a>
					</li>
				</c:if>
				<c:forEach begin="${startBlock }" end="${endBlock }" var="i">
					<li>
						<a href="info_search.do?page=${i }&&mno=-1&&accept=1&&req_box=${req_box}&&req_text=${req_text}"
						id="${i }">${i }</a>
					</li>
				</c:forEach>
				<c:if test="${endBlock < allPage }">
					<li>
						<a href="info_search.do?page=${endBlock+1 }&&mno=-1&&accept=1&&req_box=${req_box}&&req_text=${req_text}"
						>▶</a>
					</li>
					<li>
						<a href="info_search.do?page=${allPage }&&mno=-1&&accept=1&&req_box=${req_box}&&req_text=${req_text}">▶▶</a>
					</li>
				</c:if>
			</ul>
		</div>
