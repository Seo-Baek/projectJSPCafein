		 ...
		 style="margin-top: 7%; margin-bottom: 4%;">
			...
			<c:if test="${!empty board_list }">
				...
				<tr class="contlist">
					...
					<td>
						<img class=${i } 
						  onclick="location.href='goWorMCont.do?board_no=${dto.getBoard_no() }'"
					...
					<c:if test="${dto.getBoard_step()>0 }">
						...
						<%-- onclick="getCont(this.getAttribute('class'))" --%>
						 href="goWorMCont.do?board_no=${dto.getBoard_no() }"
					...
					<td style="text-align: left;">
						...
						<a class="${i }" href='goWorMCont.do?board_no=${dto.getBoard_no() }'>
			...
			<ul class="pagination">
				<c:if test="${page > block }">
					<li class="paginate_button previous">
						<a href="enter_list.do?page=1&&mno=${userNo }&&accept=0">◀◀</a>
					</li>
					<li>
						<a href=
						"enter_list.do?page=${startBlock-1 }&&mno=${userNo }&&accept=0"
							>◀</a>
					</li>
				</c:if>
				<c:forEach begin="${startBlock }" end="${endBlock }" var="i">
					<li>
						<a href="enter_list.do?page=${i }&&mno=${userNo }&&accept=0"
						id="${i }">${i }</a>
					</li>
				</c:forEach>
				<c:if test="${endBlock < allPage }">
					<li>
						<a href="enter_list.do?page=${endBlock+1 }&&mno=${userNo }&&accept=0"
						>▶</a>
					</li>
					<li>
						<a href="enter_list.do?page=${allPage }&&mno=${userNo }&&accept=0">▶▶</a>
					</li>
				</c:if>
			</ul>
		</div>
